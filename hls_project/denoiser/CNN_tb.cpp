...
#include <cmath>
int main(){
	fxp  OutModel0[(1, 28, 28)];
	fxp* Weights = (fxp*)malloc(665 * sizeof(fxp));
	float tmp;
	...
	fclose(Input);
	//Read Label
	fxp*Label = (fxp*)malloc((NumberOfPicture * (1, 28, 28)) * sizeof(fxp));
	FILE* Output = fopen("Y.txt", "r");
	for (int i = 0; i < NumberOfPicture * (1, 28, 28) ; i++)
	{
		fscanf(Output, "%f", &tmp);
		*(Label + i) = tmp;
	}
	fclose(Output);
	fxp*OutArray = (fxp*)malloc((NumberOfPicture * (1, 28, 28)) * sizeof(fxp));
	fxp Image[d * 28 * 28] = {};
	...
	float accuracy = (float)((countTrue / (NumberOfPicture * (1, 28, 28))) * 100);
